...
class FlowControlAction(BasicSampler, IncludesElements, Renderable):
    ...
    TEMPLATE = 'flow_control_action_template'
    ...
    def logical_action_on_thread(self, value):
        if not isinstance(value, ActionOnThread):
            raise TypeError(f'arg: logical_action_on_thread should be ActionOnThread. {type(value).__name__} was given')
    ...
    def pause(self, value):
        if not isinstance(value, int):
            raise TypeError(f'arg: pause should be int. {type(value).__name__} was given')
        if value < 0:
    ...
    def target(self, value):
        if not isinstance(value, Target):
            raise TypeError(f'arg: target should be Target. {type(value).__name__} was given')
        self._target = value
    ...
    def to_xml(self) -> str:
        ...
        for element in list(element_root):
            try:
                ...
                logging.error(f'Unable to render xml from {type(self).__class__}')
